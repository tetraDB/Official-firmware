...
#include "tw_apps/app_SpiritLevel.h"
 ...
 * @param style_hint
 */
void AppSpiritLevel::draw_icon(uint8_t canvasid, int16_t _pos_x, int16_t _pos_y, uint8_t style_hint)
{
	if (!is_icon_cached)
	{
		is_icon_cached = true;
 ...
 * 
 */
bool AppSpiritLevel::process_touch(touch_event_t touch_event)
{
 ...
 * 
 */
void AppSpiritLevel::draw(bool force)
{
 ...
 *
 */
void AppSpiritLevel::drawCalibrate()
{
	float mag_x;
	float mag_y;
	float mag_z;
 ...
 * 
 */
void AppSpiritLevel::drawCompass()
{
...
AppSpiritLevel app_spirit_level;
